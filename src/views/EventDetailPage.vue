    ...
    <div class="bg-black text-white min-h-screen">
      ...
      <div class="relative max-w-4xl mx-auto px-4 -mt-16 z-10 loca">
        <img :src="event?.locandina" alt="Locandina"
             class="w-full h-auto object-cover rounded-lg shadow-lg hover:shadow-xl transition-transform duration-500
  hover:scale-105"/>
      </div>
        ...
        <div class="mb-4">
          ...
          <div class="relative">
            <input
              ...
              :class="[
                'w-full px-4 py-2 bg-gray-900/50 border rounded-lg text-white placeholder-gray-500 transition-colors',
                referralValidation.isValid === true ? 'border-green-500 focus:border-green-400' :
                referralValidation.isValid === false ? 'border-red-500 focus:border-red-400' :
                'border-white/30 focus:border-orange-500'
              ]"
              placeholder="Inserisci il codice referral (es. BEE123ABC)"
              maxlength="20"
            />
            <div v-if="referralValidation.isChecking" class="absolute right-3 top-1/2 transform -translate-y-1/2">
              <i class="fas fa-spinner fa-spin text-gray-400"></i>
            </div>
            <div v-else-if="referralValidation.isValid === true" class="absolute right-3 top-1/2 transform -translate-y-1/2">
              <i class="fas fa-check text-green-500"></i>
            </div>
            <div v-else-if="referralValidation.isValid === false" class="absolute right-3 top-1/2 transform -translate-y-1/2">
              <i class="fas fa-times text-red-500"></i>
            </div>
          </div>
          <div v-if="referralValidation.message"
               :class="['text-sm mt-1',
        ...
        <div class="flex items-center justify-center">
          <div class="flex items-center -space-x-3">
            <div v-if="attendees.length > 5"
                 class="relative w-10 h-10 flex items-center justify-center rounded-full bg-gray-700 text-xs font-semibold ring-2
   ring-white"
    ...
    data() {
      return {
        ...
        API: {
          ...
          REFERRAL:      "https://api.bitcoinbeer.events/api/referral.php",
  ...
  computed: {
    ...
    methods: {
      ...
      validateReferralCode() {
        ...
        // Controllo formato base (almeno 3 caratteri, solo alfanumerici)
        if (!/^[A-Z0-9]{3,20}$/i.test(code)) {
          this.referralValidation = {
            isValid: false,
            isChecking: false,
            message: "Formato non valido. Usa solo lettere e numeri (3-20 caratteri)."
          };
          return;
        }

        // Debounce la validazione API
        this.referralValidation.isChecking = true;
        this.referralValidation.message = "Verifica del codice...";

        this.validationTimer = setTimeout(async () => {
          await this.checkReferralCodeAPI(code);
        }, 500);
      },

      async checkReferralCodeAPI(code) {
        try {
          const headers = {
          ...
          if (this.API.API_KEY) {
            headers["X-API-KEY"] = this.API.API_KEY;
          }

          const response = await fetch(`${this.API.REFERRAL}?action=validate_code&code=${encodeURIComponent(code)}`, {
            method: "GET",
            headers
          });
          ...
          if (response.status === 401) {
            this.referralValidation = {
              isValid: false,
              isChecking: false,
              message: "Errore di autenticazione API"
            };
            return;
          }
          ...
          if (response.status === 404) {
            this.referralValidation = {
              isValid: false,
              isChecking: false,
              message: "Codice referral non trovato"
            };
            return;
          }

          if (!response.ok) {
            // Fallback per altri errori HTTP
            this.referralValidation = {
              isValid: true,
              isChecking: false,
              message: "Codice accettato (validazione non disponibile)"
            };
            return;
          }
          ...
          if (data.success) {
            this.referralValidation = {
              isValid: true,
              isChecking: false,
              message: data.message || "Codice valido!"
            };
          } else {
            this.referralValidation = {
              isValid: false,
              isChecking: false,
              message: data.error || "Codice non valido o scaduto"
            };
          }
        } catch (e) {
          console.error("Errore validazione referral:", e);

          // Fallback: se l'API non è disponibile, accetta il codice se ha formato corretto
          this.referralValidation = {
            isValid: true,
            isChecking: false,
            message: "Codice accettato (validazione offline)"
          };
        }
      },
      ...
      checkReferralFromURL() {
        ...
        if (refCode) {
          // Pulisci e valida il codice dall'URL
          const cleanCode = refCode.trim().toUpperCase();
          if (/^[A-Z0-9]{3,20}$/i.test(cleanCode)) {
            this.referralCode = cleanCode;
            // Valida automaticamente il codice
            this.validateReferralCode();

            // Rimuovi il parametro dall'URL per pulizia (opzionale)
      ...
      async handleFree() {
        ...
        try {
          ...
          // ✅ CORRETTO: Aggiungi codice referral se presente e valido
          if (this.referralCode.trim() && this.referralValidation.isValid === true) {
            payload.invited_by = this.referralCode.trim().toUpperCase();
          }
    ...
    async created() {
      ...
      // Controlla se c'è un codice referral nell'URL
